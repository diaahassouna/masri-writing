...
(function () {
  ...
  Promise.all([
    ...
    fetch('assets/data/dts.json').then(r => r.json()).catch(() => null)
  ])
    .then(([RULES, DTS]) => {
      ...
      // DTS (Diaa Transliteration System) is Masri Tier 2's own alphabet
      ...
      // turned-A hamza) — see assets/data/dts.json and masri_dts.html.
      // Building the map here (rather than hardcoding it) keeps DTS output
      // permanently in sync with that JSON.
      function buildDtsMap(rules, dts) {
        const map = {};
        if (!dts || !dts.overrides) return map;
        for (const upperKey of Object.keys(dts.overrides)) {
          const ov = dts.overrides[upperKey];
          map[upperKey] = ov.dts_upper;
          const entry = rules.alphabet.find(l => l.letter_upper === upperKey);
          const lowerKey = entry ? entry.letter_lower : null;
          if (lowerKey && lowerKey !== upperKey) map[lowerKey] = ov.dts_lower;
        }
        return map;
      }
      const DTS_MAP = buildDtsMap(RULES, DTS);
      function applyDts(s) {
        if (!s) return s;
        let out = '';
        for (const ch of s) out += (DTS_MAP[ch] !== undefined) ? DTS_MAP[ch] : ch;
        return out;
      }
      ...
      // Renders either plain Tier 2 output, or (in DTS mode) the same
      // Tier 2 output with the keyboard-safe DTS substitutes swapped in —
      // origin highlighting is computed on the underlying Tier 2 text first
      // (since that's what the origin-* classes key off of), then the DTS
      ...
      function renderOutput(tier2Text, highlight) {
        ...
        return dtsMode ? applyDts(html) : html;
      }
      ...
      function update() {
        ...
        lastConverted = dtsMode ? applyDts(tier2Text) : tier2Text;
      ...
      // DTS Romanization. DTS reuses the exact same Tier 2 conversion (it's
      ...
      // 'academic' and 'dts' — dtsMode just layers the character swap and
      // chrome on top.
      const tierBadge = document.getElementById('tierBadge');
      const legendCharsEls = document.querySelectorAll('.legend-chars');
      function updateDtsChrome() {
        if (tierBadge) tierBadge.textContent = dtsMode ? 'DTS' : 'Tier 2';
        legendCharsEls.forEach(el => {
          const orig = el.getAttribute('data-tier2');
          el.textContent = dtsMode ? orig.split(' ').map(ch => DTS_MAP[ch] || ch).join(' ') : orig;
        });
      }
      ...
      modeButtons.forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          document.getElementById('masri-app').classList.toggle('mode-dts', dtsMode);
          updateDtsChrome();
      ...
      function updateName() {
        ...
        lastName = dtsMode ? applyDts(tier2Name) : tier2Name;
