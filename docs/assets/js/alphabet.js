...
(function () {
  ...
  const STRINGS = {
    en: {
      card_no: 'No.', card_listen: 'Listen', card_details: 'Details',
      ...
      col_franco: 'Franco', col_arabic: 'Arabic', col_masri: 'Masri',
      vt_arabicv: 'Arabic-V', vt_sound: 'Sound', vt_masri: 'Masri', vt_example: 'Example',
      tm_same: 'same as Tier 2', tm_name: 'Letter name'
    },
    ar: {
      card_no: 'رقم', card_listen: 'استمع', card_details: 'تفاصيل',
      ...
      col_franco: 'فرانكو', col_arabic: 'عربي', col_masri: 'مصري',
      vt_arabicv: 'Arabic-V', vt_sound: 'الصوت', vt_masri: 'مصري', vt_example: 'مثال',
      tm_same: 'زي Tier 2', tm_name: 'اسم الحرف'
    }
  };
  ...
  function originDotClass(origin) {
    ...
    if (origin === 'Extra') return 'dot-extra';
    return 'dot-ascii';
  }
  ...
  // -----------------------------------------------------------------
  // Windows keyboard layout gallery (shipped Tier 2 MSKLC layout) —
  // no data needed, just swaps the image + alt text per shift state.
  // -----------------------------------------------------------------
  const KBD_LAYER_IMAGES = {
    base: { src: 'assets/img/keyboard/tier2-win-base.jpg', alt_en: 'Masri Tier 2 Windows keyboard layout — base layer', alt_ar: 'لوحة مفاتيح مصري Tier 2 لويندوز — الطبقة الأساسية' },
    shift: { src: 'assets/img/keyboard/tier2-win-shift.jpg', alt_en: 'Masri Tier 2 Windows keyboard layout — Shift layer', alt_ar: 'لوحة مفاتيح مصري Tier 2 لويندوز — طبقة Shift' },
    capslock: { src: 'assets/img/keyboard/tier2-win-capslock.jpg', alt_en: 'Masri Tier 2 Windows keyboard layout — Caps Lock layer', alt_ar: 'لوحة مفاتيح مصري Tier 2 لويندوز — طبقة Caps Lock' },
    'shift-altgr': { src: 'assets/img/keyboard/tier2-win-shift-altgr.jpg', alt_en: 'Masri Tier 2 Windows keyboard layout — Shift + AltGr layer', alt_ar: 'لوحة مفاتيح مصري Tier 2 لويندوز — طبقة Shift + AltGr' }
  };
  document.querySelectorAll('.kbd-layer-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      document.querySelectorAll('.kbd-layer-tab').forEach(tb => tb.classList.remove('active'));
      tab.classList.add('active');
      const layer = KBD_LAYER_IMAGES[tab.dataset.layer];
      const img = document.getElementById('kbdLayerImg');
      if (layer && img) {
        img.src = layer.src;
        img.alt = LANG === 'ar' ? layer.alt_ar : layer.alt_en;
      }
    });
  });
  ...
  // -----------------------------------------------------------------
  // Tier 1 <-> Tier 2 mapping. This is NOT independent data — it is
  // transcribed directly from the live conversion rules in viral.js
  // (convertWordEveryday / convertWord), so it can never silently drift
  // from what the converter actually does. Keys are Tier 2 letter_upper.
  // "casual"/"formal" distinguishes the two Tier 1 spellings the
  // converter itself produces for θ/ð depending on the formal toggle.
  // Tier 1 never uses a digit as a letter: ع is the backtick, the emphatics
  // are the plain letter + ^ (s^ d^ t^ z^ h^ — h^ is ح, the emphatic h; plain h
  // stays ه). The glottal stop is ' (mid-word/final only). A hyphen splits a would-be digraph (Mas-h^oor). Franco digits are input only.
  // -----------------------------------------------------------------
  const TIER1_FOR_TIER2 = {
    X: 'kh', Ɣ: 'gh', Ϣ: 'sh',
    Θ: { formal: 'th', casual: 's (casual)' },
    Ð: { formal: 'dh', casual: 'z (casual)' },
    Ⲵ: '` (backtick)', Ϩ: 'h^', Ṭ: 't^', Ẓ: 'z^', Ṣ: 's^', Ḍ: 'd^', Ɐ: "'"
  };
  function tier1For(letterUpper) {
    const v = TIER1_FOR_TIER2[letterUpper];
    if (!v) return null;
    if (typeof v === 'string') return v;
    return `${v.formal} / ${v.casual}`;
  }

  Promise.all([
    fetch('assets/data/alphabet.json').then(r => r.json()),
    fetch('assets/data/drs.json').then(r => r.json()).catch(() => ({ overrides: {} }))
  ])
    .then(([DATA, DRS_DATA]) => {
      const DRS_OVERRIDES = DRS_DATA.overrides || {};
      function drsFor(letterUpper, letterLower) {
        const o = DRS_OVERRIDES[letterUpper];
        if (!o) return null; // null = identical to Tier 2, not a special DRS form
        return (letterUpper === letterLower) ? o.drs_upper : `${o.drs_upper}/${o.drs_lower}`;
      }

      const ALL_LETTERS = DATA.alphabet.concat(DATA.extra_letters);
      ...
      function renderGrid() {
        ...
        const filtered = ALL_LETTERS.filter(l => {
          ...
          const haystack = (l.name + ' ' + l.ipa + ' ' + l.arabic + ' ' + l.letter_upper + ' ' + l.letter_lower + ' ' + (l.unicode_upper || '') + ' ' + (l.unicode_lower || '') + ' ' + l.notes + ' ' + (l.notes_ar || '')).toLowerCase();
          const matchesSearch = !q || haystack.includes(q);
          return matchesOrigin && matchesSearch;
        });
        grid.innerHTML = filtered.map(l => {
          const tier1 = tier1For(l.letter_upper);
          const drs = drsFor(l.letter_upper, l.letter_lower);
          // Layer 1 (always visible): glyph, Arabic equivalent, name, sound
          // — "what letters do I need?". Layer 2 (behind the Details
          // toggle): Unicode codepoints, Tier 1/DRS substitutes, and
          // notes — the technical depth, one click away rather than
          // dumped on every card by default. When actively searching,
          // details start expanded since the match may be in that layer.
          const detailsOpen = !!q;
          return `
          <div class="letter-card" data-letter="${escapeHtml(l.letter_upper)}">
            ...
            <div class="lc-front-actions">
              ...
              <button class="lc-expand-btn" type="button" aria-expanded="${detailsOpen}">${escapeHtml(t('card_details'))} <span class="lc-expand-chevron">${detailsOpen ? '▴' : '▾'}</span></button>
            </div>
            <div class="lc-details"${detailsOpen ? '' : ' hidden'}>
              <div class="lc-unicode">${escapeHtml(l.unicode_upper)}${l.unicode_upper !== l.unicode_lower ? ' / ' + escapeHtml(l.unicode_lower) : ''}</div>
              ${(tier1 || drs) ? `<div class="lc-tiers">${tier1 ? `<span class="lc-tier-chip"><b>T1</b> ${escapeHtml(tier1)}</span>` : ''}${drs ? `<span class="lc-tier-chip"><b>DRS</b> ${escapeHtml(drs)}</span>` : ''}</div>` : ''}
        ...
        grid.querySelectorAll('.lc-expand-btn').forEach(btn => {
          btn.addEventListener('click', () => {
            const card = btn.closest('.letter-card');
            const details = card.querySelector('.lc-details');
            const nowOpen = details.hidden; // about to open
            details.hidden = !nowOpen;
            btn.setAttribute('aria-expanded', String(nowOpen));
            btn.querySelector('.lc-expand-chevron').textContent = nowOpen ? '▴' : '▾';
          });
        });
      }
      ...
      // ---------------------------------------------------------------
      // Franco numerals table — the digit→Arabic pairs are Franco's own
      // convention (not part of Masri's data), but the Arabic→Masri side
      // is looked up live from ALL_LETTERS by its Arabic letter, so this
      // table stays in sync with any future edit to the Masri alphabet.
      // ---------------------------------------------------------------
      const FRANCO_DIGITS = [
        { digit: '2', arabic: 'ء' },
        { digit: '3', arabic: 'ع' },
      ...
      const francoTable = document.getElementById('francoTable');
      if (francoTable) {
        const rows = FRANCO_DIGITS.map(f => {
          const letter = ALL_LETTERS.find(l => l.arabic === f.arabic);
          if (!letter) return '';
          return `<tr>
            <td class="franco-num">${escapeHtml(f.digit)}</td>
            <td class="franco-arrow">→</td>
            <td class="franco-arabic">${escapeHtml(letter.arabic)}</td>
            <td class="franco-arrow">→</td>
            <td class="franco-masri">${escapeHtml(letter.letter_upper)}</td>
            <td class="franco-name">${escapeHtml(letter.name)}</td>
          </tr>`;
        }).join('');
        francoTable.innerHTML = `<thead><tr>
          <th>${escapeHtml(t('col_franco'))}</th><th></th><th>${escapeHtml(t('col_arabic'))}</th><th></th><th>${escapeHtml(t('col_masri'))}</th><th></th>
      ...
      // Vowel tables
      // ---------------------------------------------------------------
      // Columns: Arabic-V form | sound (IPA) | Masri letter(s) | example. The same
      // vowel_system feeds the DRS page (which renders only the first three).
      function fillVowelTable(id, rows, cols) {
        const el = document.getElementById(id);
        if (!el) return;
        const heads = [t('vt_arabicv'), t('vt_sound'), t('vt_masri'), t('vt_example')];
        const cls = ['vt-arabic', 'vt-sound', 'vt-masri', 'vt-example'];
        el.innerHTML = `<thead><tr>${cols.map((c, i) => `<th>${escapeHtml(heads[i])}</th>`).join('')}</tr></thead>` +
          `<tbody>${rows.map(r => `<tr>${cols.map((c, i) => `<td class="${cls[i]}">${escapeHtml(r[c] || '')}</td>`).join('')}</tr>`).join('')}</tbody>`;
      }
      const soundCol = LANG === 'ar' ? 'sound_ar' : 'sound';
      fillVowelTable('shortVowelTable', DATA.vowel_system.short_vowels, ['arabic', soundCol, 'tier2', 'example']);
      fillVowelTable('longVowelTable', DATA.vowel_system.long_vowels, ['arabic', soundCol, 'tier2', 'example']);
      fillVowelTable('diphthongTable', DATA.vowel_system.diphthongs, ['arabic', soundCol, 'spelling', 'example']);

      // ---------------------------------------------------------------
      // Tier 1 <-> Tier 2 <-> DRS mapping table — only the letters where
      // at least one representation actually differs from Tier 2 are
      // listed; every other letter is identical across all three.
      // ---------------------------------------------------------------
      const tierMapEl = document.getElementById('tierMapTable');
      if (tierMapEl) {
        const rows = ALL_LETTERS
          .map(l => ({ l, tier1: tier1For(l.letter_upper), drs: drsFor(l.letter_upper, l.letter_lower) }))
          .filter(r => r.tier1 || r.drs)
          .map(r => `<tr>
            <td class="tm-arabic">${escapeHtml(r.l.arabic)}</td>
            <td class="tm-t2">${escapeHtml(r.l.letter_upper)}${r.l.letter_upper !== r.l.letter_lower ? '/' + escapeHtml(r.l.letter_lower) : ''}</td>
            <td class="tm-t1">${r.tier1 ? escapeHtml(r.tier1) : escapeHtml(r.l.letter_upper) + ' <span class="tm-same">(' + escapeHtml(t('tm_same')) + ')</span>'}</td>
            <td class="tm-drs">${r.drs ? escapeHtml(r.drs) : '<span class="tm-same">(' + escapeHtml(t('tm_same')) + ')</span>'}</td>
            <td class="tm-name">${escapeHtml(r.l.name)}</td>
          </tr>`).join('');
        tierMapEl.innerHTML = `<thead><tr>
          <th>${escapeHtml(t('col_arabic'))}</th><th>Tier 2</th><th>Tier 1</th><th>DRS</th><th>${escapeHtml(t('tm_name'))}</th>
        </tr></thead><tbody>${rows}</tbody>`;
      }
