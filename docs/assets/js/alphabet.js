...
(function () {
  ...
  Promise.all([
    ...
    fetch('assets/data/dts.json').then(r => r.json()).catch(() => ({ overrides: {} }))
  ])
    .then(([DATA, DTS_DATA]) => {
      const DTS_OVERRIDES = DTS_DATA.overrides || {};
      function dtsFor(letterUpper, letterLower) {
        const o = DTS_OVERRIDES[letterUpper];
        if (!o) return null; // null = identical to Tier 2, not a special DTS form
        return (letterUpper === letterLower) ? o.dts_upper : `${o.dts_upper}/${o.dts_lower}`;
      }
      ...
      function renderGrid() {
        ...
        grid.innerHTML = filtered.map(l => {
          ...
          const dts = dtsFor(l.letter_upper, l.letter_lower);
          return `
          <div class="letter-card" data-letter="${escapeHtml(l.letter_upper)}">
            ...
            ${(tier1 || dts) ? `<div class="lc-tiers">${tier1 ? `<span class="lc-tier-chip"><b>T1</b> ${escapeHtml(tier1)}</span>` : ''}${dts ? `<span class="lc-tier-chip"><b>DTS</b> ${escapeHtml(dts)}</span>` : ''}</div>` : ''}
      ...
      // ---------------------------------------------------------------
      // Tier 1 <-> Tier 2 <-> DTS mapping table — only the letters where
      ...
      if (tierMapEl) {
        const rows = ALL_LETTERS
          .map(l => ({ l, tier1: tier1For(l.letter_upper), dts: dtsFor(l.letter_upper, l.letter_lower) }))
          .filter(r => r.tier1 || r.dts)
          .map(r => `<tr>
            ...
            <td class="tm-dts">${r.dts ? escapeHtml(r.dts) : '<span class="tm-same">(' + escapeHtml(t('tm_same')) + ')</span>'}</td>
            <td class="tm-name">${escapeHtml(r.l.name)}</td>
          </tr>`).join('');
        tierMapEl.innerHTML = `<thead><tr>
          <th>${escapeHtml(t('col_arabic'))}</th><th>Tier 2</th><th>Tier 1</th><th>DTS</th><th>${escapeHtml(t('tm_name'))}</th>
        </tr></thead><tbody>${rows}</tbody>`;
      }
